...
const gameOverSign = document.getElementById("game-over"); //notificacion de fin

//configuraciones del juego
const boardSize = 10; //tamanio del tablero
const gameSpeed = 200; //velocidad, milisegundos
const squareTypes = {
  emptySquare: 0,
  snakeSquare: 1,
  foodSquare: 2
...
const moveSnake = () => {
  const newSquare = String(
    Number(snake[snake.length - 1]) + directions[direction]).padStart(2, "0"); //toma el ultimo cuadrado de la snake y a ella le hace una suma (+) la info de la direccion (1, -1, 10 etc). Ej: 02+1= 03, ES DECIR SE MOVIO A LA DERECHA UN CUADRADO
 //como aca especificamos asi ArrowRight: 1, ArrowLeft: -1,debemos agregarle el 0 al inicio y que se formen las coordenadas.
  const [row, column] = newSquare.split('');

  if (
    newSquare < 0 || //se ubica en un cuadrado menor que el valor 0, es imposible.
    ...
    if (boardSquares[row][column] === squareTypes.foodSquare) {
      ...
      const emptySquare = snake.shift(); //le sacamos el primer cuadrado a la serpiente;
      drawSquare(emptySquare, "emptySquare"); //avisamos que ahora ese lugar es uno vacio, no mas ocupado
    }
...
const directionEvent = (key) => {
  //recibe la tecla que tocamos.
  switch (
    key.code //tecla pulsada
  ) {
    case "ArrowUp":
...
const createRandomFood = () => {
  const randomEmptySquare =
    emptySquares[Math.floor(Math.random() * emptySquares.length)]; //crea una coordenada random entre arrays.
...
const createBoard = () => {
  boardSquares.forEach((row, rowIndex) => {
    row.forEach((column, columnIndex) => {
      ...
      squareElement.setAttribute("class", "square emptySquare"); //le agregamos clases a ese div.
...
const startGame = () => {
  ...
  gameOverSign.style.display = "none"; //ocultamos por las dudas aca tambien
